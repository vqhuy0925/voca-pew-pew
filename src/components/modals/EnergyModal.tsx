...
export const EnergyModal: React.FC<EnergyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2.5 sm:p-4 bg-black/85 backdrop-blur-md select-none animate-in fade-in duration-200">
      <div
        className={`relative w-full max-w-xl bg-gradient-to-b ${theme.bgGradient} border-2 sm:border-3 ${theme.borderAccent} rounded-3xl p-3.5 sm:p-5 text-white max-h-[92vh] flex flex-col overflow-hidden shadow-2xl`}
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2.5 mb-2.5 flex-shrink-0">
          <div className="min-w-0 flex-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-yellow-500/20 border border-yellow-400/50 rounded-full text-yellow-300 text-[11px] sm:text-xs font-extrabold uppercase">
              <Zap className="w-3.5 h-3.5 fill-yellow-400" /> Trạm Năng Lượng
            </div>
        ...
        {/* Tab Navigation */}
        <div className="grid grid-cols-3 p-1 bg-slate-950/80 rounded-2xl border border-slate-800 mb-3 flex-shrink-0">
          <button
            type="button"
            onClick={() => {
              soundFx.playClick();
            ...
            className={`py-1.5 px-2 rounded-xl font-game font-black text-xs sm:text-sm flex items-center justify-center gap-1.5 transition cursor-pointer ${
              activeTab === 'status'
        ...
        <div className="mt-2 pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2.5 flex-shrink-0">
          ...
          <button
            type="button"
            onClick={onClose}
            className="py-2 sm:py-2.5 px-4 sm:px-6 bg-gradient-to-r from-yellow-400 to-amber-500 hover:from-yellow-300 hover:to-amber-400 text-slate-950 font-game font-extrabold text-xs sm:text-sm rounded-xl sm:rounded-2xl border-b-4 border-amber-700 active:border-b-0 active:translate-y-1 transition shadow-lg flex items-center gap-1.5 cursor-pointer flex-shrink-0"
          >
            <span>XONG RỒI! 🎮</span>
          </button>
        </div>
